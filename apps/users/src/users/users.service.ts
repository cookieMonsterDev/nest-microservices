...
export class UsersService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    await this.kafkaService.createConsumer('users-consumer', ['users-service'], async ({ message }) => {
      ...
      switch (data.action) {
        ...
        case 'create':
          ...
          break;
        case 'update':
          const userIndex = this.users.findIndex((u) => u.id === data.data.id);
          if (userIndex !== -1) {
          ...
          break;
        case 'delete':
          const deleteIndex = this.users.findIndex((u) => u.id === data.data.id);
          if (deleteIndex !== -1) {
